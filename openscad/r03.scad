...
module part2_RotaryHandle(loc_res = 32, see_into = 1) {
    difference() {
        hull() {
            translate([0, 0, 0])
            cylinder(d = 50, h = 12.7-2.5-.4, $fn= 16);
            translate([0, 0, 0])
            cylinder(d = 45, h = 12.7-1.4, $fn = loc_res);
        }
        // middle/axis cutout
        difference() {
            translate([0, 0, -1])
...
module part4_LightRing(loc_res = 32) {
    ...
    difference() {
        ...
        translate([0, 0, 4])
        cylinder(d2 = 29.5+15, d1 = 29.5, h = 3+1, $fn = loc_res);
        // LED cutouts
        rled = 18;
        nbleds = 8;
        translate([0, 0, 0])
...
module pcbR02(loc_res = 32, show = 1) {
    if(show) {
        ...
        for(n = [0:1:nbbuttons-1]) {
            ...
            rotate([0, 180, 90+360/nbbuttons*n])
